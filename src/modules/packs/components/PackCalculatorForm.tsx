  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-2 max-w-xl">
      ...
      <button
        ...
        className="w-fit px-4 py-2 bg-emerald-600 text-white rounded hover:bg-emerald-700 disabled:opacity-50"
      >
        {calc.isPending ? 'Calculating...' : 'Calculate'}
      </button>
      {calc.isError && <div className="text-red-600">{calc.error.message}</div>}
    </form>
  )
}
